...
async function seedUniversities() {
  try {
    ...
    // Find admin user
    const adminUser = await Usuario.findOne({ email: 'admin@garrote.com' });
    if (!adminUser) {
    ...
    for (const [codigo, data] of Object.entries(universitiesData)) {
      ...
      const universidad = new Universidad({
        codigo,
        nombre: data.nombre,
        tipo: 'publica', // Required field
        ciudad: 'Valencia', // Required field
        activa: true, // Correct field name
      ...
      for (let i = 0; i < data.titulaciones.length; i++) {
        ...
        const titulacion = new Titulacion({
          ...
          creadoPor: adminUser._id, // Required field
          orden: i + 1,
          duracion: 4, // Add required field
          creditos: 240 // Add required field
        });
